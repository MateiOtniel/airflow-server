...
with DAG(
    dag_id = "accesa_discount_analysis",
    catchup = False,
    ...
    spark_job = SparkSubmitOperator(
        task_id = 'run_accesa_discount_analysis',
        application = 'scripts/spark_jobs/accesa_discount_analysis.py',
        conn_id = 'spark_default',
        application_args = [
            "--project", GCP_PROJECT_ID,
            "--dataset", "accesa_analytics",
            "--table", "accesa_discount_analysis",
            "--temp-bucket", BUCKET,
        ],
        conf = {
    ...
    mail = EmailOperator(
        ...
        subject = 'Accesa Discount Analysis done',
        html_content = '<p>Discount analytics done.</p>'
    )

    [wait_products, wait_discounts, wait_transactions, wait_customers, wait_prices] >> spark_job >> mail
