...
with DAG(
    dag_id = "accesa_customer_metrics",
    catchup = False,
    ...
    spark_job = SparkSubmitOperator(
        task_id = 'run_accesa_customer_metrics',
        application = 'scripts/spark_jobs/accesa_customer_metrics.py',
        conn_id = 'spark_default',
        application_args = [
            "--project", GCP_PROJECT_ID,
            "--dataset", "accesa_analytics",
            "--table", "accesa_customer_metrics",
            "--temp-bucket", BUCKET,
        ],
        conf = {
    ...
    mail = EmailOperator(
        ...
        subject = 'Accesa Customer Metrics done',
        html_content = '<p>Customer-level metrics done.</p>'
    )

    [wait_customers, wait_products, wait_transactions, wait_prices] >> spark_job >> mail
