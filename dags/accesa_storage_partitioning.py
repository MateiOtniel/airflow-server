...
with DAG(
    dag_id = "accesa_storage_partitioning",
    catchup = False,
    ...
    spark_job = SparkSubmitOperator(
        task_id = 'run_accesa_storage_partitioning',
        application = 'scripts/spark_jobs/accesa_storage_partitioning.py',
        conn_id = 'spark_default',
        application_args = [
            "--project", GCP_PROJECT_ID,
            "--dataset", "accesa_analytics",
            "--table", "accesa_storage_partitioning",
            "--temp-bucket", BUCKET,
        ],
        conf = {
    ...
    mail = EmailOperator(
        ...
        subject = 'Accesa Storage Partitioning done',
        html_content = '<p>Parquet/CSV partitions + top customers text written.</p>'
    )

    [wait_products, wait_prices, wait_transactions, wait_customers] >> spark_job >> mail
