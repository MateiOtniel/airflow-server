...
with DAG(
    ...
    spark_job = SparkSubmitOperator(
        task_id = 'run_accesa_recommendations',
        application = 'scripts/spark_jobs/accesa_recommendations.py',
        conn_id = 'spark_default',
        application_args = [
            "--project", GCP_PROJECT_ID,
            "--dataset", "accesa_analytics",
            "--table", "accesa_recommendations",
            "--temp-bucket", BUCKET,
        ],
        conf = {
    ...
    mail = EmailOperator(
        ...
        subject = 'Accesa Recommendations done',
        html_content = '<p>Next-product recommendations computed.</p>'
    )

    [wait_customers, wait_products, wait_transactions] >> spark_job >> mail
